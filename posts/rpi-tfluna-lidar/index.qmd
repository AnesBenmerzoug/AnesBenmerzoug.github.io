...
::: {.callout-note title="EDIT 2025-08-29"}
After publishing the original version of the post, I realized that the user manual 
I was using was out-of-date and that there was a [newer one available](https://en.benewake.com/DataDownload/index_pid_20_lcid_21.html) on the Benewake website.
...
team, called *OTO* which is a homonym for *Auto.* an abbreviation for *Automatique* (Automation in French), that built many different embedded projects like [a ball and beam system](https://en.wikipedia.org/wiki/Ball_and_beam), a ball and plate system and a [persistence of vision](https://en.wikipedia.org/wiki/Persistence_of_vision) (PoV) display.
...
Unfortunately for me, once I received the TF-Luna and read its [user manual](https://en.benewake.com/DataDownload/index_pid_20_lcid_21.html) I quickly realized that it is 1D LiDAR, i.e. that it only measures the distance
...
![Schematics of ToF Principle - [TF-Luna user manual](https://files.seeedstudio.com/wiki/Grove-TF_Mini_LiDAR/res/SJ-PM-TF-Luna-A03-Product-Manual.pdf)](time_of_flight_tfluna.svg)
...
: Parameters specification of TF-Luna - [TF-Luna user manual](https://en.benewake.com/DataDownload/index_pid_20_lcid_21.html) {#tbl-tfluna-parameters}
...
- The [TF-Luna user manual](https://files.seeedstudio.com/wiki/Grove-TF_Mini_LiDAR/res/SJ-PM-TF-Luna-A03-Product-Manual.pdf).
...
from the user manual for reading the sensor measurements. This task should have been easy but it was made hard
...
ever I wrote **0** to the **0x25** register to enable it. It only worked after I decided randomly to try
writing **1** instead. After investigating for a bit, I realized that I was using an
[older version](https://files.seeedstudio.com/wiki/Grove-TF_Mini_LiDAR/res/SJ-PM-TF-Luna-A03-Product-Manual.pdf)
of the user manual (`A03` from 2020.3.15) instead of the newer version (`A05` from 2020.7.23).

In this newer version, the definition of I2C register **0x25** was changed so that to **enable** (respectively **disable**)
the LiDAR sensor we have to set the **0x25** register to **1** (respectively **0**) instead of **0** (respectively **1**), see @fig-user-manual-error.

![Excerpt from page 30 of TF-Luna user manual version `A03` showing values that were changed in version `A05`](tfluna_user_manual_error.svg){#fig-user-manual-error}
...
It has quite a lot of features and will increase the size of the final binary, but I didn't bother for now 
to go through them and determine which ones are absolutely necessary and which ones are not.
...
Thanks to this project, I have learned about using Rust on a Raspberry Pi and about dealing with not so intuitive sensor documents.
